...
so they run offline from a fresh clone. The tests sit next to the code they test and find
these files through the `@test/*` path in `tsconfig.json`, e.g.
`import.meta.resolve('@test/fixtures/nestest/nestest.log')`.

## `nestest/`

- `nestest.log` – Reference trace of kevtris's CPU test ROM running in automation mode
  (PC = `$C000`), produced with Nintendulator. The ROM itself is `public/roms/nestest.nes`,
  which the app loads on startup and the tests use too.
...
matches blargg's NES-verified tests instead, see `src/systems/nes/core/cpu.singlestep.test.ts`.
...
`6-MMC3_alt` are expected to fail, see `src/systems/nes/core/mappers/mapper_004.blargg.test.ts`.
...
partway through a block move; `src/systems/snes/core/cpu.blockmove.test.ts` covers them instead.
...
cycle counts aren't checked, see `src/systems/snes/core/apu/spc700.singlestep.test.ts`.
